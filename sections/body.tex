...
\ref{sec:methods}), though it is possible to use any cloud provider that
provides Kubernetes support, such as Google Cloud, Microsoft Azure, or even
institutional hardware at many universities.
...
order to run Jupyter notebooks that are hosted remotely on the cloud.
...
This bootcamp used the XSEDE cluster for managing resources in the cloud. This is
because XSEDE is widely available to many universities in the United States. However,
...
top of lower-level cloud computing technologies (such as Open Stack), we required
minimal setup and expertise in connecting these tools with the resources in the cloud.
This allowed instructors and organizers to spend more of their time focusing on course
material.
...
the JupyterHub <<<CITATION?>>> project is being developed in parallel with
